...
# MAGIC FROM
# MAGIC   IDENTIFIER(:catalog || '.silver.km_appointments') AS aptmt
# MAGIC   LEFT JOIN IDENTIFIER(:catalog || '.silver.km_matrix') AS matrix ON matrix.appointmentid = aptmt.id
# MAGIC WHERE
...
# MAGIC        org.name,
# MAGIC        users.mail
# MAGIC FROM IDENTIFIER(:catalog || '.silver.km_orgusermapping') AS map
# MAGIC LEFT JOIN IDENTIFIER(:catalog || '.silver.km_kmusers') AS users ON users.id = map.kmuserid
# MAGIC LEFT JOIN IDENTIFIER(:catalog || '.silver.km_orgs') AS org ON map.orgId = org.id
...
# MAGIC %sql
# MAGIC SELECT * 
# MAGIC FROM IDENTIFIER(:catalog || '.silver.km_kmusers')
